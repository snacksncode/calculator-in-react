...
export const formatValue = (value: string) => {
  switch (value) {
    ...
    case "%":
      return (
        <svg xmlns="http://www.w3.org/2000/svg" fill="currentColor" className="w-5 h-5" viewBox="0 0 384 512">
...
export const CalculatorButton = ({ value, afterClick, pressedKeys, colored = false, bgColored = false }: Props) => {
  ...
  return (
    <button
      ...
      className={`p-3 text-2xl leading-none transition-colors font-medium flex items-center justify-center rounded-full ${getColors()}`}
    >
      {formatValue(value)}
    </button>
  );
